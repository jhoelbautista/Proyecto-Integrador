...
const HistorialAcademico = () => {
  ...
  const handleConsultarHistorial = (estudiante) => {
    alert(`Consultar Historial Académico de: ${estudiante.nombres} ${estudiante.apellidoPaterno}`);
    // Aquí podrías redirigir o mostrar un modal con el historial académico
  };
            ...
            filteredEstudiantes.map((est) => (
              <tr key={est.id} className="hover:bg-gray-50">
                ...
                <td className="py-3 px-5 border-b">
                  <button
                    onClick={() => handleConsultarHistorial(est)}
                    className="bg-blue-600 text-white px-4 py-2 rounded-md text-sm hover:bg-blue-700 transition-all border border-blue-400"
                  >
